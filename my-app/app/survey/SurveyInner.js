...
export default function SurveyInner() {
  ...
  const matchedToolCount = responses.filter(
    (resp, i) => resp === "yes" && algoMappings[i]
  ).length;
  ...
  const updateURLWithResponses = (responsesArray) => {
    const compact = responsesArray
      .map((r) => (r === "yes" ? 1 : r === "no" ? 0 : "x"))
      .join("-");
  ...
  useEffect(() => {
    ...
    if (ans) {
      const restored = ans
        .split("-")
        .map((val) => (val === "1" ? "yes" : val === "0" ? "no" : null));
  ...
  useEffect(() => {
    const allAnswered =
      responses.length === questions.length && responses.every((r) => r);
    if (allAnswered) {
      setShowSurvey(false);
      setTimeout(() => {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      <div className="text-center mb-6">
        ...
        <p className="text-lg">
          Answer 17 <em>yes</em> or <em>no</em> questions to access your
          personalized NYC AlgoMatcher report
        </p>
      </div>
      ...
      {answeredCount === questions.length && (
        <div ref={resultsRef} className="h-4">
          <div className="mt-16 text-left border-t pt-10">
            <div className="text-center mb-6">
              <h2 className="text-3xl font-bold mb-2">
                Your NYC AlgoMatcher Report
              </h2>
              <p className="text-lg">
                These are the{" "}
                <span className="font-semibold">{matchedToolCount}</span>{" "}
                algorithmic tools potentially using your data:
              </p>
            </div>
